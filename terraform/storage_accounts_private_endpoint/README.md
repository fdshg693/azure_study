...
### エラー: Storage Account名が既に使用されている

```
Error: A resource with the ID already exists
```

**解決策**: `variables.tf` で `storage_account_name` を別の一意な名前に変更

### エラー: Blobのアップロードに失敗

```
Error: uploading blob: authorization error
```

**原因**: `public_network_access_enabled = false` の場合、Terraformの実行環境からアクセスできない

**解決策**:
1. 一時的に `public_network_access_enabled = true` に変更
2. または、TerraformをVNet内のVMから実行

### VMにSSH接続できない

**確認事項**:
1. NSGルールでSSH（ポート22）が許可されているか
2. VMのパブリックIPアドレスが正しいか
3. 管理者ユーザー名とパスワードが正しいか

## 📞 サポート

問題が発生した場合:

1. [Terraform Azure Provider ドキュメント](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs)
2. [Azure Private Link ドキュメント](https://docs.microsoft.com/azure/private-link/)
3. [Azure Storage ドキュメント](https://docs.microsoft.com/azure/storage/)

## 📝 ライセンス

このサンプルコードは教育目的で提供されています。
